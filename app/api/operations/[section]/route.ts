...
export async function GET(_request: Request, context: { params: Promise<{ section: string }> }) {
  try {
    ...
    if (!principal.roles.some((role) => operationsRoles.has(role))) {
      return NextResponse.json({ error: "FORBIDDEN", detail: "Operations evidence requires an authorized control-plane role." }, { status: 403, headers: { "Cache-Control": "no-store" } });
    }

    const { section } = await context.params;
    if (!operationsSectionIds.includes(section as (typeof operationsSectionIds)[number])) {
      return NextResponse.json({ error: "NOT_FOUND", allowedSections: operationsSectionIds }, { status: 404, headers: { "Cache-Control": "no-store" } });
    }

    const result = getOperationsSection(section);
    if (!result) return NextResponse.json({ error: "NOT_FOUND" }, { status: 404, headers: { "Cache-Control": "no-store" } });
