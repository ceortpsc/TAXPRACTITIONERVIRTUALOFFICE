...
function configItem(id: string, label: string, value: string | undefined, detail: string): OperationalItem {
  ...
  return {
    ...
    source: present ? "runtime environment reference" : "required environment reference missing",
    verifiedAt: nowIso(),
  };
}
  ...
  return {
    ...
    items: [
      ...
      },
      {
        ...
        detail: commit ? commit : "No provider commit SHA is available in this runtime.",
      ...
      },
      {
        ...
        status: "READY",
        detail: "Production promotion remains approval-gated; this console does not fabricate deployment success or bypass release review.",
...
function health(): OperationsSection {
  return {
    ...
    description: "Fail-closed readiness checks for application, database, identity, tax-provider, and evidence services.",
    items: [
      {
        id: "application",
        label: "Application runtime",
        status: "VERIFIED",
        detail: `Node ${process.version}; uptime ${Math.floor(process.uptime())} seconds.`,
        evidenceClass: "RUNTIME",
        source: "process runtime",
        verifiedAt: nowIso(),
      },
      configItem("database", "Database", process.env.DATABASE_URL, "PostgreSQL system-of-record connection reference."),
      configItem("identity", "Identity", process.env.CLERK_SECRET_KEY, "Clerk server credential reference for authenticated control-plane access."),
      configItem("irs", "IRS integration boundary", process.env.IRS_CLIENT_ID, "Registered IRS client reference; absence keeps provider actions gated."),
      configItem("audit-key", "Audit signing", process.env.AUDIT_SIGNING_KEY_ID, "Managed audit signing key reference."),
    ],
  };
}
...
function optimization(): OperationsSection {
  ...
  return {
    id: "optimization",
    title: "Optimization",
    description: "Runtime efficiency signals and safe tuning recommendations without unsupported performance claims.",
    items: [
      {
        id: "heap",
  ...
  return {
    ...
    items: [
      ...
      ...workers.slice(0, 6).map((worker) => ({
        ...
        detail: `${worker.purpose} Schedule: ${worker.schedule}; execution: ${worker.execution}.`,
        evidenceClass: "DESIGN_CONTROL" as EvidenceClass,
        source: "worker registry",
        verifiedAt: nowIso(),
      })),
    ],
...
function topology(): OperationsSection {
  ...
  return {
    id: "topology",
    title: "App Topology",
    description: "Preview of the controlled application dependency graph. Topology describes approved architecture; it is not a claim that every external provider is currently online.",
    items: nodes.map((node, index) => ({
      id: `node-${index + 1}`,
      label: node,
      status: "READY",
      detail: index === 0 ? "Clients terminate at the application edge; browsers never connect directly to IRS, Treasury, banking, or transcript endpoints." : "Registered architecture node in the RTPSC controlled runtime topology.",
      evidenceClass: "DESIGN_CONTROL",
